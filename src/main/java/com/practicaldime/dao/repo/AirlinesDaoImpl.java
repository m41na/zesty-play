...
import com.practicaldime.dao.entity.AirlineMapper;
...
public class AirlinesDaoImpl implements AirlinesDao{
	...
	public AppResult<Airline> create(Airline air) {
		String sql = "insert into tbl_airlines (id, airline, alias, iata, icao, service, country, active) "
				+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        int rows = jdbcTemplate.update(sql, new Object[]{air.id, air.airline, air.alias, air.iata, air.icao, air.service, air.country, air.active});
